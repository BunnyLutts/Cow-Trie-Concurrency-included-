...
namespace sjtu {
    ...
    private:
        ...
        template <class T>
        auto Put(std::shared_ptr<const TrieNode> node, std::string_view key, int pos, T &value) const -> std::shared_ptr<const TrieNode> {
            std::shared_ptr<TrieNode> new_node;
            if (pos >= key.length()) {
                if (!node) {
                    new_node = std::make_shared<TrieNodeWithValue<T>>(std::make_shared<T>(std::move(value)));
                } else {
                    new_node = std::make_shared<TrieNodeWithValue<T>>(node->children_, std::make_shared<T>(std::move(value)));
                }
            } else {
                if (!node) {
                    new_node = std::make_shared<TrieNode>();
                } else {
                    new_node = std::move(node->Clone());
                }
                new_node->children_[key[pos]] = Put(new_node->children_[key[pos]], key, pos+1, value);
            }
            return new_node;
        }

        // Check before used.
        auto Remove(std::shared_ptr<const TrieNode> node, std::string_view key, int pos) const ->  std::shared_ptr<const TrieNode> {
            std::shared_ptr<TrieNode> new_node;
            if (pos >= key.length()) {
                new_node = std::make_shared<TrieNode>(node->children_);
            } else {
                new_node = std::move(node->Clone());
                new_node->children_[key[pos]] = Remove(new_node->children_[key[pos]], key, pos+1);
            }
            return new_node;
        }

        bool Find(std::string_view key) const {
            auto node = root_;
            for (auto c : key) {
    ...
    public:
        ...
        auto Get(std::string_view key) const -> const T * {
            ...
            if (node->is_value_node_) {
                return reinterpret_cast<const TrieNodeWithValue<T> *>(node.get())->value_.get();
            } else
                return nullptr;
        }
        ...
        auto Put(std::string_view key, T value) const -> Trie {
            return Trie(Put<T>(root_, key, 0, value));
        }

        // Remove the key from the trie. If the key does not exist, return the original trie.
        // Otherwise, returns the new trie.
        auto Remove(std::string_view key) const -> Trie {
            if (!Find(key)) return *this;
            else return Trie(Remove(root_, key, 0));
        }
    };
